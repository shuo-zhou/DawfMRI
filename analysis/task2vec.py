...
def task2vec(task_file='Brain_func.csv'):
    ...
    task_vecs = np.zeros((len(id_ar), len(func_list)))
    
    for i in range(len(id_ar)):
        for f in func_dict[id_ar[i]]:
            task_vecs[i, func_list.index(f)] = 1
    vec_df = pd.DataFrame(task_vecs, index = id_ar, columns = func_list)
    vec_df.to_csv('task_vecs.csv')
    return task_vecs, id_ar, func_list
