...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 20 16:28:20 2017
...
def main():
    config = commandline()
    script_name='bet_%s.sh'%config.dataset
    script_file =open (script_name,'w')
    bold=glob.glob(os.path.join(config.basedir,config.dataset,'sub*/BOLD/*/bold_mcf.nii.gz'))

    if len(bold)==0:
        print 'no bold images found!'
        print 'please process motion correction first!'
        config.printHelp()
        return
    #write command to script file    
    for i in bold:
        
        script_file.write('bet %s %s -F\n'%(i,i.replace('.nii','_brain.nii')))
    script_file.close()
  
    full_path = os.path.dirname(os.path.realpath(__file__))
    if config.qsub:
        qsub_name = 'bet_%s_qsub.sh'%config.dataset
        qsub_file = open(qsub_name,'w')
        commands = open(script_name,'r')
        i=0
        for command in commands:
            sub_name = 'bet_%s_qsub_%s.sh'%(config.dataset,i)
